...
async function resolveNotFound(request: Request): Promise<Response> {
	...
	if (fallback === 'true') {
		...
		return new Response(result.body, {headers: {'Content-Type': 'image/png', 'Cache-Control': 'public, max-age=3600, must-revalidate'}});
	}
	...
	if (fallback) {
		...
		if (contentTypeFromFallback?.startsWith('image/')) {
			...
			return new Response(result.body, {headers: {'Content-Type': contentTypeFromFallback, 'Cache-Control': 'public, max-age=3600, must-revalidate'}});
		}
	}
...
export async function GET(request: Request, context: TContext): Promise<Response> {
	...
	if (result.ok) {
		if (fileName.endsWith('.svg')) {
			return new Response(result.body, {headers: {'Content-Type': 'image/svg+xml', 'Cache-Control': 'public, max-age=3600, must-revalidate'}});
		}
		return new Response(result.body, {headers: {'Content-Type': 'image/png', 'Cache-Control': 'public, max-age=3600, must-revalidate'}});
	}
